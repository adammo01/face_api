...
def _set_settings(values: dict) -> dict:
    allowed = {
        ...
        # --- 实验室同步到全局 (2026-08-20): 模式/条数/彩铅 ---
        "mode": (None, None, str),
        "modes": (None, None, list),
        "bar_count": (2, 10, int),
        "bar_span": (0.3, 0.95, float),
        "sigma_s": (1.0, 200.0, float),
        "sigma_r": (0.01, 1.0, float),
        "shade_factor": (0.0, 1.0, float),
        "saturation_boost": (0.0, 3.0, float),
        "paper_strength": (0.0, 1.0, float),
    }
    saved = {}
    with _db() as conn:
        for key, raw in values.items():
            ...
            # JSON 文本: modes/face_profiles/mode (mode 也存 JSON 文本, 否则 _get_blur_default
            # 的 json.loads 无法解析裸字符串 "green_red_bars", 全局回显/回退会失效)
            if key in ("face_profiles", "modes", "mode"):
                val = raw
...
class FaceBlurRequest(BaseModel):
    ...
    mode: str = Field(
        "gaussian",
        pattern=r"^(pixelate|gaussian|solid|landmark|landmark_whole_face|green_red_bars|color_pencil|landmark_whole_face_v1|landmark_whole_face_v2|landmark_whole_face_v3)$",
    )
    ...
    bar_count: int = Field(5, ge=2, le=10, description="绿框红条条数")
    bar_span: float = Field(0.76, ge=0.3, le=0.95, description="绿框红条纵向覆盖范围比例")
    sigma_s: float = Field(60.0, ge=1.0, le=200.0, description="彩铅平滑尺度")
    sigma_r: float = Field(0.07, ge=0.01, le=1.0, description="彩铅边缘保留强度")
    shade_factor: float = Field(0.05, ge=0.0, le=1.0, description="彩铅阴影强度")
    saturation_boost: float = Field(1.3, ge=0.0, le=3.0, description="彩色饱和度倍率")
    paper_strength: float = Field(0.15, ge=0.0, le=1.0, description="纸张纹理强度")
    noise_strength: float = Field(0.02, ge=0.0, le=0.2, description="细微噪点强度")
    min_face_skip: int = Field(10, ge=0, le=500, description="极小人脸跳过阈值")
...
<body>
        ...
        <div class="lab-params">
          ...
          <div class="lab-params-body">
            <div class="lab-param">
              ...
              <div id="lab-mode" class="lab-mode-select">
                ...
                <div id="lab-mode-options" class="lab-mode-options" hidden>
                  ...
                  <label><input type="checkbox" value="color_pencil" onchange="labModeChanged(this)" /> 彩色铅笔画（整图）</label>
                </div>
              </div>
            ...
            <div class="lab-param">
              ...
              <div class="hint">按原始脸宽(px)命中一档：21-39、40-60、60-80、80-110。每档的 modes 和参数独立生效；未命中时使用上面的全局模式。</div>
  ...
  "grid_n": 5
            ...
            <div class="lab-param"><label>跳小脸(px) <span id="lab-minface-v">10</span></label><input type="range" id="lab-minface" min="0" max="500" step="5" value="10" oninput="document.getElementById('lab-minface-v').textContent=this.value" /><div class="hint">脸宽小于此值直接跳过，0=全部打码</div></div>
            ...
            <div class="lab-param"><label>红条条数 <span id="lab-barcount-v">5</span></label><input type="number" id="lab-barcount" min="2" max="10" step="1" value="5" oninput="document.getElementById('lab-barcount-v').textContent=this.value" /><div class="hint">绿框红条条数 (2-10)</div></div>
            ...
            <div id="lab-pencil-params" hidden>
              <div class="lab-param"><label>平滑尺度 sigma_s <span id="lab-sigma-s-v">60</span></label><input type="number" id="lab-sigma-s" min="1" max="200" step="1" value="60" oninput="document.getElementById('lab-sigma-s-v').textContent=this.value" /><div class="hint">越大笔触越平滑，处理更柔和。</div></div>
              <div class="lab-param"><label>边缘保留 sigma_r <span id="lab-sigma-r-v">0.07</span></label><input type="number" id="lab-sigma-r" min="0.01" max="1" step="0.01" value="0.07" oninput="document.getElementById('lab-sigma-r-v').textContent=this.value" /><div class="hint">越小越强调轮廓线。</div></div>
              <div class="lab-param"><label>阴影 shade_factor <span id="lab-shade-v">0.05</span></label><input type="number" id="lab-shade" min="0" max="1" step="0.01" value="0.05" oninput="document.getElementById('lab-shade-v').textContent=this.value" /><div class="hint">彩铅阴影浓淡。</div></div>
              <div class="lab-param"><label>饱和度倍率 <span id="lab-saturation-v">1.30</span></label><input type="number" id="lab-saturation" min="0" max="3" step="0.05" value="1.3" oninput="document.getElementById('lab-saturation-v').textContent=Number(this.value).toFixed(2)" /><div class="hint">保留彩色效果，1.0 为不增强。</div></div>
              <div class="lab-param"><label>纸张纹理强度 <span id="lab-paper-v">0.15</span></label><input type="number" id="lab-paper" min="0" max="1" step="0.01" value="0.15" oninput="document.getElementById('lab-paper-v').textContent=Number(this.value).toFixed(2)" /><div class="hint">纸张颗粒叠加比例。</div></div>
            </div>
            <div class="lab-btns">
...
const LAB_DEFAULTS = {mode:"landmark_whole_face", modes:["landmark_whole_face"], face_profiles:[], score_threshold:0.52, expand_ratio:0.30, min_face_skip:10, face_grid_step:14, dot_radius:3, grid_n:5, bar_count:5, bar_span:0.76, sigma_s:60, sigma_r:0.07, shade_factor:0.05, saturation_boost:1.3, paper_strength:0.15};
...
function labToggleMode(){
  ...
  const pencil = m.includes("color_pencil");
  ...
  document.getElementById("lab-pencil-params").hidden = !pencil;
  const labels = {landmark_whole_face:"整脸红点遮罩", landmark:"关键点遮罩", landmark_whole_face_v1:"方案v1 · K6同步密度", landmark_whole_face_v2:"方案v2 · 线上默认", landmark_whole_face_v3:"方案v3 · K4更密", green_red_bars:"绿框红条（可调条数）", color_pencil:"彩色铅笔画（整图）", pixelate:"马赛克", gaussian:"高斯模糊", solid:"纯色遮挡"};
...
function labModeChanged(input){
  if(input.value === "color_pencil" && input.checked){
    document.querySelectorAll('#lab-mode-options input').forEach(o => { if(o !== input) o.checked = false; });
  } else if(input.value !== "color_pencil" && input.checked){
    const pencil = document.querySelector('#lab-mode-options input[value="color_pencil"]');
    if(pencil) pencil.checked = false;
  }
...
function labCurrentParams(){
  ...
  return {mode:modes[0]||"gaussian", modes, face_profiles, score_threshold:Number(document.getElementById("lab-score").value), expand_ratio:Number(document.getElementById("lab-expand").value), min_face_skip:Number(document.getElementById("lab-minface").value), face_grid_step:Number(document.getElementById("lab-step").value), dot_radius:Number(document.getElementById("lab-dot").value), grid_n:Number(document.getElementById("lab-n").value), bar_count:Number(document.getElementById("lab-barcount").value), bar_span:Number(document.getElementById("lab-barspan").value), sigma_s:Number(document.getElementById("lab-sigma-s").value), sigma_r:Number(document.getElementById("lab-sigma-r").value), shade_factor:Number(document.getElementById("lab-shade").value), saturation_boost:Number(document.getElementById("lab-saturation").value), paper_strength:Number(document.getElementById("lab-paper").value)};
}
function labSetParams(params){
  const values = Object.assign({}, LAB_DEFAULTS, params || {});
  ["mode","score_threshold","expand_ratio","min_face_skip","face_grid_step","dot_radius","grid_n","bar_count","bar_span","sigma_s","sigma_r","shade_factor","saturation_boost","paper_strength"].forEach(name => {
    const id = {mode:"lab-mode", score_threshold:"lab-score", expand_ratio:"lab-expand", min_face_skip:"lab-minface", face_grid_step:"lab-step", dot_radius:"lab-dot", grid_n:"lab-n", bar_count:"lab-barcount", bar_span:"lab-barspan", sigma_s:"lab-sigma-s", sigma_r:"lab-sigma-r", shade_factor:"lab-shade", saturation_boost:"lab-saturation", paper_strength:"lab-paper"}[name];
  ...
  document.getElementById("lab-sigma-s-v").textContent = values.sigma_s;
  document.getElementById("lab-sigma-r-v").textContent = Number(values.sigma_r).toFixed(2);
  document.getElementById("lab-shade-v").textContent = Number(values.shade_factor).toFixed(2);
  document.getElementById("lab-saturation-v").textContent = Number(values.saturation_boost).toFixed(2);
  document.getElementById("lab-paper-v").textContent = Number(values.paper_strength).toFixed(2);
  labToggleMode();
  labModeUserTouched = false;
  document.getElementById("lab-sync-btn").disabled = true;
}
...
async function labTest(){
  ...
  try {
    const body = {mode: m, modes: params.modes, face_profiles: params.face_profiles,
      ...
      sigma_s: Number(document.getElementById("lab-sigma-s").value),
      sigma_r: Number(document.getElementById("lab-sigma-r").value),
      shade_factor: Number(document.getElementById("lab-shade").value),
      saturation_boost: Number(document.getElementById("lab-saturation").value),
      paper_strength: Number(document.getElementById("lab-paper").value),
    };
...
async function labSyncGlobal(){
  ...
  const m = params.modes[0] || "gaussian";
  const body = {
    mode: m, modes: params.modes, face_profiles: params.face_profiles,
    score_threshold: Number(document.getElementById("lab-score").value),
    expand_ratio: Number(document.getElementById("lab-expand").value),
    min_face_skip: Number(document.getElementById("lab-minface").value),
    face_grid_step: Number(document.getElementById("lab-step").value),
...
def lab_test(req: FaceBlurRequest, request: Request, authorization: str | None = Header(default=None), x_admin_token: str | None = Header(default=None)):
    ...
    blur_params.update({
        "sigma_s": req.sigma_s, "sigma_r": req.sigma_r,
        "shade_factor": req.shade_factor,
        "saturation_boost": req.saturation_boost,
        "paper_strength": req.paper_strength,
        "noise_strength": req.noise_strength,
    })
    if req.mode == "landmark_whole_face":
        blur_params.update({
            "adaptive": False, "min_face_skip": req.min_face_skip if req.min_face_skip is not None else 10,
...
def face_blur(req: FaceBlurRequest, request: Request):
    task_id = uuid.uuid4().hex
    # 快照: _normalize_model 内部 setattr 会原地改写 req.model_fields_set, 必须先拷贝 (2026-08-20 T-A/B/C)
    explicit = set(req.model_fields_set)
    model_cfg = _normalize_model(req)
    # 全局生效 modes:
    #  - 显式 modes 优先
    #  - 仅"纯普通请求"(mode/modes 均未显式) 才回退 DB settings 全局 modes
    #  - 显式 mode 请求保持原语义, 不被 DB 全局覆盖 (2026-08-20 T-A/B/C 保守改法)
    if "modes" in explicit and req.modes:
        _eff_modes = req.modes
    elif "mode" not in explicit:
        _eff_modes = _get_blur_default("modes", model_cfg.get("modes", [req.mode]))
    else:
        _eff_modes = model_cfg.get("modes", [req.mode])
    # 归一化: DB/显式 modes 里的预设名 (landmark_whole_face_v1/v2/v3) 转 base_mode,
    # 否则 process_image 校验会 ValueError -> 400 (2026-08-20 P1)
    _eff_modes = [BLUR_MODELS.get(m, {}).get("base_mode", m) for m in _eff_modes]
    ...
    effective_params = {
        ...
        "min_face_skip": req.min_face_skip if "min_face_skip" in explicit else int(model_cfg.get("min_face_skip", _get_blur_default("min_face_skip", 10))),
        ...
        "bar_count": req.bar_count if "bar_count" in explicit else int(model_cfg.get("bar_count", _get_blur_default("bar_count", 5))),
        "bar_span": req.bar_span if "bar_span" in explicit else float(model_cfg.get("bar_span", _get_blur_default("bar_span", 0.76))),
        # 彩铅参数全局生效 (2026-08-20 P2): 与 bar_count/bar_span 相同回退模式
        "sigma_s": req.sigma_s if "sigma_s" in explicit else float(model_cfg.get("sigma_s", _get_blur_default("sigma_s", 60.0))),
        "sigma_r": req.sigma_r if "sigma_r" in explicit else float(model_cfg.get("sigma_r", _get_blur_default("sigma_r", 0.07))),
        "shade_factor": req.shade_factor if "shade_factor" in explicit else float(model_cfg.get("shade_factor", _get_blur_default("shade_factor", 0.05))),
        "saturation_boost": req.saturation_boost if "saturation_boost" in explicit else float(model_cfg.get("saturation_boost", _get_blur_default("saturation_boost", 1.3))),
        "paper_strength": req.paper_strength if "paper_strength" in explicit else float(model_cfg.get("paper_strength", _get_blur_default("paper_strength", 0.15))),
        "noise_strength": req.noise_strength,
        "modes": _eff_modes,
...
def _face_blur_impl(task_id: str, req: FaceBlurRequest, request: Request, *, cache_key: str = "", effective_params: dict | None = None):
    ...
    try:
        ...
        for key in ("sigma_s", "sigma_r", "shade_factor", "saturation_boost", "paper_strength", "noise_strength"):
            if key in params:
                blur_params[key] = params[key]
...
def admin_summary(
    ...
    return {
        ...
        "settings": {
            ...
            "min_face_skip": _get_blur_default("min_face_skip", 10),
...
class AdminSettingsRequest(BaseModel):
    ...
    # --- 实验室同步到全局 (2026-08-20): 校验范围与 FaceBlurRequest 对齐 ---
    mode: Optional[str] = Field(
        default=None,
        pattern=r"^(pixelate|gaussian|solid|landmark|landmark_whole_face|green_red_bars|color_pencil|landmark_whole_face_v1|landmark_whole_face_v2|landmark_whole_face_v3)$",
    )
    modes: Optional[list[str]] = Field(default=None, max_length=5)
    bar_count: Optional[int] = Field(default=None, ge=2, le=10)
    bar_span: Optional[float] = Field(default=None, ge=0.3, le=0.95)
    sigma_s: Optional[float] = Field(default=None, ge=1.0, le=200.0)
    sigma_r: Optional[float] = Field(default=None, ge=0.01, le=1.0)
    shade_factor: Optional[float] = Field(default=None, ge=0.0, le=1.0)
    saturation_boost: Optional[float] = Field(default=None, ge=0.0, le=3.0)
    paper_strength: Optional[float] = Field(default=None, ge=0.0, le=1.0)
...
def admin_get_settings(
    ...
    return {
        ...
        "settings": {
            ...
            "grid_n": _get_blur_default("grid_n", 5),
            "face_profiles": _get_blur_default("face_profiles", []),
            "mode": _get_blur_default("mode", "gaussian"),
            "modes": _get_blur_default("modes", ["gaussian"]),
            "bar_count": _get_blur_default("bar_count", 5),
            "bar_span": _get_blur_default("bar_span", 0.76),
            "sigma_s": _get_blur_default("sigma_s", 60.0),
            "sigma_r": _get_blur_default("sigma_r", 0.07),
            "shade_factor": _get_blur_default("shade_factor", 0.05),
            "saturation_boost": _get_blur_default("saturation_boost", 1.3),
            "paper_strength": _get_blur_default("paper_strength", 0.15),
            "inflight_tasks": inflight,
        },
    }
...
def admin_set_settings(
    ...
    if any(key in values for key in ("score_threshold", "expand_ratio", "min_face_skip", "dot_radius", "face_grid_step", "grid_n", "face_profiles", "mode", "modes", "bar_count", "bar_span", "sigma_s", "sigma_r", "shade_factor", "saturation_boost", "paper_strength")):
        _cache_clear()
        _bump_cache_epoch()
        with _db() as conn:
            conn.execute("DELETE FROM blur_cache")
...
<div class="tab-view" data-tab="settings" hidden>
  ...
  <script>
    ...
    async function loadSummary(){
      ...
      document.getElementById('s-minface').value = s.min_face_skip ?? 10;
