...
def _task_slot():
    ...
    limit = _get_int_setting("max_concurrent_tasks", DEFAULT_MAX_CONCURRENT_TASKS, 1, 32)
    with _INFLIGHT_LOCK:
...
def _download(url: str, max_bytes: int = MAX_IMAGE_BYTES,
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "faceblur-api/1.0"})
...
def face_blur(req: FaceBlurRequest, request: Request):
    ...
    try:
        with _task_slot():
            return _face_blur_impl(task_id, req, request)
...
def _face_blur_impl(task_id: str, req: FaceBlurRequest, request: Request):
...
def admin_summary(
    ...
    return {
        ...
        "service": {
            ...
            "max_concurrent_tasks": _get_int_setting("max_concurrent_tasks", DEFAULT_MAX_CONCURRENT_TASKS, 1, 32),
        },
        "requests": {
            "total": total,
            "ok": ok,
...
def admin_files(
    ...
    _require_admin(request, authorization, x_admin_token, token)
    items, total = _static_files(offset=offset, limit=limit)
...
class AdminSettingsRequest(BaseModel):
    max_concurrent_tasks: Optional[int] = Field(default=None, ge=1, le=32)
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .split { display: grid; grid-template-columns: minmax(0, 1.4fr) minmax(300px, .8fr); gap: 14px; }
    ...
    .task-id { max-width: 210px; overflow: hidden; color: var(--muted); font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
...
<body>
  <main class="shell">
    ...
    <div class="task-search">
      <input id="task-search" type="search" placeholder="输入任务 ID 定位请求" aria-label="任务 ID" />
    ...
    <section class="split">
      <div class="panel">
        ...
        <table>
          <thead><tr><th>时间</th><th>任务 ID</th><th>状态</th><th>模式</th><th>来源</th><th>耗时</th><th>操作</th></tr></thead>
  ...
  <script>
    ...
    function saveToken(){ localStorage.setItem('faceblur_admin_token', tokenEl.value); loadAll(); }
    ...
    function imageView(id, label, url){
      return `<div class="image-preview"><h3>${label}</h3>${url
        ? `<img id="${id}" src="${escapeHtml(url)}" alt="${label}" loading="lazy" />`
        : `<div class="image-empty" id="${id}">无可预览图片</div>`}</div>`;
    }
    async function loadRequests(){
      const d = await api(`/api/admin/requests?offset=${(requestPage - 1) * requestPageSize}&limit=${requestPageSize}`);
    ...
    async function loadFiles(){
      ...
      document.getElementById('files').innerHTML = d.items.map(x => `
        <div class="file"><img src="${x.url}" loading="lazy" /><div>${x.name}<br>${fmtBytes(x.size)}</div></div>`).join('') || '<div class="status">暂无图片</div>';
    }
      ...
      try {
        ...
        document.getElementById('task-detail').innerHTML = `
          ...
          <div class="task-images">
            ${imageView('task-input-image', '输入图片', inputUrl)}
            ${imageView('task-output-image', sameImage ? '结果图片（未检测到人脸，返回原图）' : '生成结果', outputUrl)}
          </div>
          <div class="task-json">
